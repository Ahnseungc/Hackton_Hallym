...
export const ChatElement: React.FC<ChatElementProps> = (props) => {
  const { data } = useSWR<ItemOutputDto>(
    "https://d1xx291z7z5z7p.cloudfront.net/item/" + props.itemId,
    fetcher
  );
  const chat = useSWR<ChatOutputDto[]>(
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {chat.data[chat.data.length - 1].message}
          </span>
        </div>
      </div>
    </Link>
