...
const ChatDetail: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://d1xx291z7z5z7p.cloudfront.net/item/${searchParams.get(
          "itemId"
        )}`
      )
      .then((r) => {
  ...
  const { data, isLoading } = useSWR<ChatOutputDto[]>(
    `https://d1xx291z7z5z7p.cloudfront.net/chat`,
    async (url) => {
  ...
  return (
    <div style={{ paddingTop: "10px", width: "100%" }}>
      <Link
        href={"productdetail/" + searchParams.get("itemId")}
        style={{
          textDecoration: "none",
        }}
      >
      ...
      >
        <input
          ...
          onKeyDown={async (k) => {
            if (k.key == "Enter") {
              await axios.post<ChatOutputDto, ChatOutputDto, ChatInputDto>(
                "https://d1xx291z7z5z7p.cloudfront.net/chat",
                {
                  itemId: +searchParams.get("itemId"),
                  buyer: +searchParams.get("buyerId"),
